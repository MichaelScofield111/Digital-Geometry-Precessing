...
{  
    return MRGB(static_cast<unsigned int>(color.r * 255.0f),  
                static_cast<unsigned int>(color.g * 255.0f),  
                static_cast<unsigned int>(color.b * 255.0f));  
}  
...
{  
    // 假设MRGBA的alpha通道是0-255的范围  
    // 注意：如果color.a的范围是[0, 1]，则应该乘以255，而不是100  
    return MRGBA(static_cast<unsigned int>(color.r * 255),  
                  static_cast<unsigned int>(color.g * 255),  
                  static_cast<unsigned int>(color.b * 255),  
                  // 这里假设MRGBA的alpha通道是0-255，因此乘以255  
                  static_cast<unsigned int>(color.a * 255)); // 修改这里  
}
